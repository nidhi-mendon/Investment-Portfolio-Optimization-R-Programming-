...
######## Hult MFIN - Cohort 1
####################################
######## Modeling and Analytics - Optimized Portfolio
####################################
...
stock3 <- getSymbols("GOOGL", auto.assign = FALSE)
stock4 <- getSymbols("NFLX", auto.assign = FALSE)
stock5 <- getSymbols("HD", auto.assign = FALSE)
stock6 <- getSymbols("DIS", auto.assign = FALSE)
stock7 <- getSymbols("WMT", auto.assign = FALSE)

##### Step 2 - Combining all stocks into one dataframe
joined_prices <- merge.xts(stock1, stock2, stock3, stock4, stock5, stock6, stock7)

##### We just need the adjusted prices: [only stands for adjusted price only]
joined_prices_only <- joined_prices[,seq(from = 6 , to = 42 , by = 6)]
...
stock3_returns_daily <- dailyReturn(getSymbols("GOOGL", auto.assign = FALSE))
stock4_returns_daily <- dailyReturn(getSymbols("NFLX", auto.assign = FALSE))
stock5_returns_daily <- dailyReturn(getSymbols("HD", auto.assign = FALSE))
stock6_returns_daily <- dailyReturn(getSymbols("DIS", auto.assign = FALSE))
stock7_returns_daily <- dailyReturn(getSymbols("WMT", auto.assign = FALSE))

joined_daily_returns <- merge.xts(stock1_returns_daily, stock2_returns_daily, stock3_returns_daily, stock4_returns_daily, 
                                  stock5_returns_daily, stock6_returns_daily, stock7_returns_daily)
...
stock3_returns_monthly <- monthlyReturn(getSymbols("GOOGL", auto.assign = FALSE))
stock4_returns_monthly <- monthlyReturn(getSymbols("NFLX", auto.assign = FALSE))
stock5_returns_monthly <- monthlyReturn(getSymbols("HD", auto.assign = FALSE))
stock6_returns_monthly <- monthlyReturn(getSymbols("DIS", auto.assign = FALSE))
stock7_returns_monthly <- monthlyReturn(getSymbols("WMT", auto.assign = FALSE))
...
joined_monthly_returns <- merge.xts(stock1_returns_monthly, stock2_returns_monthly, stock3_returns_monthly, stock4_returns_monthly, 
                                    stock5_returns_monthly, stock6_returns_monthly, stock7_returns_monthly)
...
stock3_returns_yearly <- yearlyReturn(getSymbols("GOOGL", auto.assign = FALSE))
stock4_returns_yearly <- yearlyReturn(getSymbols("NFLX", auto.assign = FALSE))
stock5_returns_yearly <- yearlyReturn(getSymbols("HD", auto.assign = FALSE))
stock6_returns_yearly <- yearlyReturn(getSymbols("DIS", auto.assign = FALSE))
stock7_returns_yearly <- yearlyReturn(getSymbols("WMT", auto.assign = FALSE))

joined_yearly_returns <- merge.xts(stock1_returns_yearly, stock2_returns_yearly, stock3_returns_yearly, stock4_returns_yearly, 
                                   stock5_returns_yearly, stock6_returns_yearly, stock7_returns_yearly)
...
### Stock Allocation:
AAPL_alloc <- 0.20000000
MSFT_alloc <- 0.05776043
GOOGL_alloc <- 0.20000000
NFLX_alloc <- 0.02953387
HD_alloc <- 0.20000000
DIS_alloc <- 0.20000000
WMT_alloc <- 0.11270570
...
joined_monthly_returns <- as.data.frame(joined_monthly_returns)%>%
  mutate(portfolio = AAPL_alloc*joined_monthly_returns$monthly.returns+
           ...
           GOOGL_alloc*joined_monthly_returns$monthly.returns.2+
           NFLX_alloc*joined_monthly_returns$monthly.returns.3+
           HD_alloc*joined_monthly_returns$monthly.returns.4+
           DIS_alloc*joined_monthly_returns$monthly.returns.5+
           WMT_alloc*joined_monthly_returns$monthly.returns.6)
...
joined_yearly_returns <- as.data.frame(joined_yearly_returns)%>%
  mutate(portfolio = AAPL_alloc*joined_yearly_returns$yearly.returns+
           ...
           GOOGL_alloc*joined_yearly_returns$yearly.returns.2+
           NFLX_alloc*joined_yearly_returns$yearly.returns.3+
           HD_alloc*joined_yearly_returns$yearly.returns.4+
           DIS_alloc*joined_yearly_returns$yearly.returns.5+
           WMT_alloc*joined_yearly_returns$yearly.returns.6)
...
GOOGL_sigma <- sd(joined_monthly_returns$monthly.returns.2[time_index : (time_index-11)])*sqrt(12)
NFLX_sigma <- sd(joined_monthly_returns$monthly.returns.3[time_index : (time_index-11)])*sqrt(12)
HD_sigma <- sd(joined_monthly_returns$monthly.returns.4[time_index : (time_index-11)])*sqrt(12)
DIS_sigma <- sd(joined_monthly_returns$monthly.returns.5[time_index : (time_index-11)])*sqrt(12)
WMT_sigma <- sd(joined_monthly_returns$monthly.returns.6[time_index : (time_index-11)])*sqrt(12)
...
AAPL_te <- sd(joined_monthly_returns$monthly.returns[time_index : (time_index-11)]-
                joined_monthly_returns$monthly.returns.7[time_index : (time_index-11)])*sqrt(12)
MSFT_te <- sd(joined_monthly_returns$monthly.returns.1[time_index : (time_index-11)]-
                joined_monthly_returns$monthly.returns.7[time_index : (time_index-11)])*sqrt(12)
GOOGL_te <- sd(joined_monthly_returns$monthly.returns.2[time_index : (time_index-11)]-
                 joined_monthly_returns$monthly.returns.7[time_index : (time_index-11)])*sqrt(12)
NFLX_te <- sd(joined_monthly_returns$monthly.returns.3[time_index : (time_index-11)]-
                joined_monthly_returns$monthly.returns.7[time_index : (time_index-11)])*sqrt(12)
HD_te <- sd(joined_monthly_returns$monthly.returns.4[time_index : (time_index-11)]-
              joined_monthly_returns$monthly.returns.7[time_index : (time_index-11)])*sqrt(12)
DIS_te <- sd(joined_monthly_returns$monthly.returns.5[time_index : (time_index-11)]-
               joined_monthly_returns$monthly.returns.7[time_index : (time_index-11)])*sqrt(12)
WMT_te <- sd(joined_monthly_returns$monthly.returns.6[time_index : (time_index-11)]-
               joined_monthly_returns$monthly.returns.7[time_index : (time_index-11)])*sqrt(12)
...
GOOGL_sharpe <- (mean(joined_monthly_returns$monthly.returns.2[time_index:(time_index-11)])-riskfree)/sd(joined_monthly_returns$monthly.returns.2[time_index:(time_index-11)])
NFLX_sharpe <- (mean(joined_monthly_returns$monthly.returns.3[time_index:(time_index-11)])-riskfree)/sd(joined_monthly_returns$monthly.returns.3[time_index:(time_index-11)])
HD_sharpe <- (mean(joined_monthly_returns$monthly.returns.4[time_index:(time_index-11)])-riskfree)/sd(joined_monthly_returns$monthly.returns.4[time_index:(time_index-11)])
DIS_sharpe <- (mean(joined_monthly_returns$monthly.returns.5[time_index:(time_index-11)])-riskfree)/sd(joined_monthly_returns$monthly.returns.5[time_index:(time_index-11)])
WMT_sharpe <- (mean(joined_monthly_returns$monthly.returns.6[time_index:(time_index-11)])-riskfree)/sd(joined_monthly_returns$monthly.returns.6[time_index:(time_index-11)])
...
#### CAPM
AAPL_reg <- lm(monthly.returns ~ monthly.returns.6, data=last_24_months) #alternatively data = joined_monthly_returns
summary(AAPL_reg)
MSFT_reg <- lm(monthly.returns.1 ~ monthly.returns.6, data=last_24_months)
summary(MSFT_reg)
GOOGL_reg <- lm(monthly.returns.2 ~ monthly.returns.6, data=last_24_months)
summary(GOOGL_reg)
NFLX_reg <- lm(monthly.returns.3 ~ monthly.returns.6, data=last_24_months)
summary(NFLX_reg)
HD_reg <- lm(monthly.returns.4 ~ monthly.returns.6, data=last_24_months)
summary(HD_reg)
DIS_reg <- lm(monthly.returns.5 ~ monthly.returns.6, data=last_24_months)
summary(DIS_reg)
WMT_reg <- lm(monthly.returns.5 ~ monthly.returns.6, data=last_24_months)
summary(WMT_reg)
Portfolio_reg <- lm(portfolio~ monthly.returns.6, data=last_24_months) 
...
GOOGL_Treynor <- (mean(joined_monthly_returns$monthly.returns.2[(time_index-23):time_index])-riskfree)/
  GOOGL_reg$coefficients[2]
NFLX_Treynor <- (mean(joined_monthly_returns$monthly.returns.3[(time_index-23):time_index])-riskfree)/
  NFLX_reg$coefficients[2]
HD_Treynor <- (mean(joined_monthly_returns$monthly.returns.4[(time_index-23):time_index])-riskfree)/
  HD_reg$coefficients[2]
DIS_Treynor <- (mean(joined_monthly_returns$monthly.returns.5[(time_index-23):time_index])-riskfree)/
  DIS_reg$coefficients[2]
WMT_Treynor <- (mean(joined_monthly_returns$monthly.returns.6[(time_index-23):time_index])-riskfree)/
...
compare_chart <- ggplot(data=joined_prices_only)+
  ...
  geom_line(aes(x=index(joined_prices_only), y=GOOGL.Adjusted), color="violet")+
  geom_line(aes(x=index(joined_prices_only), y=NFLX.Adjusted), color="orange")+
  ...
  geom_line(aes(x=index(joined_prices_only), y=WMT.Adjusted), color="black")
print(compare_chart)
...
compare_ror <- ggplot(data=joined_monthly_returns)+
  ...
  geom_line(aes(x=index(joined_monthly_returns), y=monthly.returns.4), color="purple")+
  geom_line(aes(x=index(joined_monthly_returns), y=monthly.returns.5), color="brown")+
  geom_line(aes(x=index(joined_monthly_returns), y=monthly.returns.6), color="black")
print(compare_ror)
...
WMT_FF3F <- fama_french_3F(ticker="WMT", from_date='2010-01-31', to_date='2021-02-28')
summary(WMT_FF3F[[2]])
...
hist(joined_monthly_returns$monthly.returns)
hist(joined_monthly_returns$monthly.returns.1)
hist(joined_monthly_returns$monthly.returns.2)
hist(joined_monthly_returns$monthly.returns.3)
hist(joined_monthly_returns$monthly.returns.4)
hist(joined_monthly_returns$monthly.returns.5)
hist(joined_monthly_returns$monthly.returns.6)
